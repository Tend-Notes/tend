...
// Seed: Block content component (CodeMirror-based, with dormant HTML mode)
//
// Handles text editing within a single block using CodeMirror when active.
// When dormant, renders static HTML using the shared content renderer.
//
// Architecture: Seed is a thin wrapper that renders either ActiveSeed (CodeMirror)
// or DormantSeed (static HTML). This split avoids conditional hook calls.
//
// Seed is responsible for:
// - Text input and editing via CodeMirror (active mode)
// - Static HTML rendering with click-to-activate (dormant mode)
...
import { parseContent, renderContentReact, mapRenderedOffsetToSource } from '../contentRenderer'
...
interface SeedProps {
  ...
  codeLanguage?: string
  /** Whether this seed is active (CodeMirror) or dormant (static HTML). Defaults to true. */
  isActive?: boolean
...
// ─────────────────────────────────────────────────────────────────────────────
// DORMANT SEED
// Static HTML rendering when the seed is not being edited.
// ─────────────────────────────────────────────────────────────────────────────

/**
 * Determine the caret offset from a mouse click event within rendered text.
 * Uses caretPositionFromPoint (standard) with caretRangeFromPoint (Safari fallback).
 */
function getCaretOffsetFromClick(e: React.MouseEvent): number | undefined {
  ...
  // Walk the DOM tree to calculate the total text offset within the container
 ...
 * Handles click-to-activate and navigation for wikilinks/tags.
 */
const DormantSeed = React.memo(forwardRef<SeedHandle, {
  content: string
  isCodeBlock: boolean
  onActivate?: (cursorOffset?: number) => void
}>(function DormantSeed({ content, isCodeBlock, onActivate }, ref) {
  ...
  // Expose SeedHandle stub for dormant mode
  ...
  // Navigation handler for wikilinks and tags (matches Actions.tsx pattern)
...
// ─────────────────────────────────────────────────────────────────────────────
// ACTIVE SEED
// Full CodeMirror editor for text editing.
// ─────────────────────────────────────────────────────────────────────────────

const ActiveSeed = forwardRef<SeedHandle, {
  block: Block
  isSelected: boolean
  onChange: (content: string) => void
  onBoundaryEvent: (event: SeedBoundaryEvent) => void
  ...
  // Drag-out detection: mousedown inside, mousemove outside while button held
  useEffect(() => {
    const container = containerRef.current
    if (!container) return
...
ActiveSeed.displayName = 'ActiveSeed'
  ...
  (
    ...
    return (
      <ActiveSeed
        ...
        onChange={onChange}
        onBoundaryEvent={onBoundaryEvent}
        onFocus={onFocus}
        onBlur={onBlur}
        readonly={readonly}
        isCodeBlock={isCodeBlock}
